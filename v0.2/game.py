...
####################################################
# Changelog:
# Made car direction change smoother
# Made enemy cars move at different speeds
# Added another class for car object
# Removed print coordinate function used for debug purposes
# Re-built check for going off race track L157
# Other small bug fixes relating to enemy cars
            ...
            for event in pygame.event.get():#Getting any event
                ...
                if event.type == pygame.KEYDOWN:#Check if key is pressed down and is kept down then
                    if event.key == pygame.K_LEFT:#if key pressed is left arrow
                        playerX_change = -10#set x motion equal to -10
                    if event.key == pygame.K_RIGHT:#if key pressed is right arrow
                        playerX_change = 10#x rate of motion equal to 10
                if event.type == pygame.KEYUP:#when that pressed key is released then
                    if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                        playerX_change = 0#rate of x motion make it equal to zero
